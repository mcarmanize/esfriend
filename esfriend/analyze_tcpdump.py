"""
    Script to analyze the contents of a tcpdump pcap

    copied mostly from: https://github.com/cuckoosandbox/cuckoo/blob/master/cuckoo/processing/network.py
...
from utility import convert_to_printable, versiontuple
...
# exception to use in this module
class EsfriendProcessingError(Exception):
    """Error in analyze_tcpdump module."""
...
RESOLVE_DNS = True
SORT_PCAP = True
RESULT_SERVER_IP = socket.gethostbyname(socket.gethostname())
...
def iplayer_from_raw(raw, linktype=1):
    ...
    if linktype == 1:  # ethernet
        ...
        raise EsfriendProcessingError("unknown PCAP linktype")
